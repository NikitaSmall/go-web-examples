...
type (
	...
	// GetTasksCommand is the command line data structure for the get action of tasks
	GetTasksCommand struct {
	}
...
// RegisterFlags registers the command flags with the command line.
func (cmd *DeleteTasksCommand) RegisterFlags(cc *cobra.Command) {
}

// Run makes the HTTP request corresponding to the GetTasksCommand command.
func (cmd *GetTasksCommand) Run(c *client.Client, args []string) error {
	var path string
	if len(args) > 0 {
		path = args[0]
	} else {
...
func (cmd *UpdateTasksCommand) Run(c *client.Client, args []string) error {
	...
	var payload app.UpdateTasksPayload
	if cmd.Payload != "" {
		err := json.Unmarshal([]byte(cmd.Payload), &payload)
		if err != nil {
			return fmt.Errorf("failed to deserialize payload: %s", err)
...
// RegisterFlags registers the command flags with the command line.
func (cmd *UpdateTasksCommand) RegisterFlags(cc *cobra.Command) {
	cc.Flags().StringVar(&cmd.Payload, "payload", "", "Request JSON body")
}
